...
//Battery Voltage level Pin
const int VOLTAGE_PIN = A1;
float voltMult = 1.0;
const float MAX_MULTIPLIER = 1.35;
...
void executeRaceLogic() {
  ...
  */
  ...
  if (leftWasBlack || rightWasBlack || activeSensors >= 3) {
    ...
    moveMotors(SPEED_CREEP * voltMult, SPEED_CREEP * voltMult);
  ...
  lastError = error;
  int currentBase = baseSpeed * voltMult;
  int currentMotorSpeed = motorSpeed * voltMult;
  int leftMotorSpeed = constrain(currentBase + currentMotorSpeed, -255, maxSpeed);
  int rightMotorSpeed = constrain(currentBase - currentMotorSpeed, -255, maxSpeed);
...
long readLine() {
  ...
  // THE NEW PHYSICAL MAP:
  int sensorWeights[6] = {3000, 1500, 500, -500, -1500, -3000};
...
void updateBatterySag() {
  ...
  // Prevent divide-by-zero or insane spikes if the pin wire comes loose
  if (rawVoltage < 600) rawVoltage = 600; 
  
  // 859 is our 12.6V baseline. 
  // As rawVoltage drops, the multiplier goes up natively!
  voltMult = 810.0 / (float)rawVoltage;
  
  if (voltMult > MAX_MULTIPLIER) {
    voltMult = MAX_MULTIPLIER;
  }
}
...
void executeTurn(char type, char dir){
  ...
  int currentTurnSpeed = constrain(SPEED_TURN * voltMult, 0, maxSpeed);

  
  // TYPE 'A': Acute Angles
  if (type == 'A') {
    if (dir == 'L') { moveMotors(-currentTurnSpeed, currentTurnSpeed); }
    else if (dir == 'R') { moveMotors(currentTurnSpeed, -currentTurnSpeed); }
